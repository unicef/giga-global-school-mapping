...
def join_with_geoboundaries(master, preds,  data_config, iso_code, threshold_dist=250, adm_level="ADM2"):
    geoboundaries = data_utils._get_geoboundaries(data_config, iso_code, adm_level="ADM2")
    intersection, master_filtered, preds_filtered = get_intersection(master, preds, threshold_dist=threshold_dist)
...
def plot_choropleth(map, column="n_points", cmap='Blues', shrink=0.6, vmax=None, vcenter=None):
    if vcenter:
...
def show_barplot(master, preds, threshold_dist=250, pad=50):
    plt.figure(dpi=1200)
    master_unconfirmed, preds_unvalidated = get_statistics(master, preds, threshold_dist)
    bins = pd.cut(preds_unvalidated["prob"], [0.5, 0.6, 0.7, 0.8, 0.9, 1], 5).value_counts().sort_index()
    ...
    ax.set_title(f"(Total: {len(preds_unvalidated):,})", loc="left", fontdict={"fontsize": 10})
...
def get_intersection(master, preds, threshold_dist=150, distance_col="distance_to_nearest_master"):
...
def get_statistics(master, preds, threshold_dist=150):
    intersection, master_filtered, preds_filtered = get_intersection(master, preds, threshold_dist)
    master_unconfirmed = master_filtered[~master_filtered["MUID"].isin(intersection["MUID"])]
    master_confirmed = master_filtered[master_filtered["MUID"].isin(intersection["MUID"])]
    preds_unvalidated = preds_filtered[~preds_filtered["PUID"].isin(intersection["PUID"])]
...
def venn_diagram(master, preds, threshold_dist=150):
    ...
    intersection, master_filtered, preds_filtered = get_intersection(master, preds, threshold_dist)
    master_unconfirmed, preds_unvalidated = get_statistics(master, preds, threshold_dist)
    subsets = (master_unconfirmed.shape[0], preds_unvalidated.shape[0], intersection.shape[0])
...
def donut_chart(master, source="master", source_col="clean"):
    ...
    code = {0: "Unique school \nlocations", 1: "Unpopulated \narea", 2: "Non-unique school \nlocations"}
...
def load_data(iso_code, data_config, model_config=None, sum_threshold=5, adm_level="ADM2", source="pred", source_col="clean"):

    def clean_data(data, source_col):
        data.loc[(data[source_col] == 1), source_col] = 0
        data.loc[(data[source_col] == 3), source_col] = 2
    ...
    if source == "master":
        ...
        data = data.rename({'UID': 'PUID'}, axis=1)
        data["PUID"] = data.index
        logging.info(f"Data dimensions: {data.shape}")
