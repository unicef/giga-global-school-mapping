    ...
    def forward(self, x):
        if self.training:
            ...
            while torch.all(mask):
                mask = (
                    torch.rand(batch_size, num_channels, 1, 1, device=x.device) > self.p
                )
            ...
            if self.signal_to_noise is not None:
                signal_level = (
                    torch.rand(batch_size, num_channels, 1, 1, device=x.device)
                    * (self.signal_to_noise[1] - self.signal_to_noise[0])
...
class RandomMask2D(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            mask = (
                torch.rand(x.size(0), 1, x.size(2), x.size(3), device=x.device) > self.p
            )
...
class ScaleSkip2D(nn.Module):
    ...
    def __init__(self, channels, drop_y=None, signal_to_noise=(0.1, 0.9), size=0.01):
        ...
        if self.drop_y is not None and self.drop_y > 0.0:
            self.drop_y = GaussianDropout2d(
                self.drop_y, signal_to_noise=signal_to_noise
            )
        else:
            self.drop_y = None
        ...
        self.set_weights()
        while (
            torch.any(self.x_skipscale == 0)
            or torch.any(self.y_skipscale == 0)
            or torch.any(self.y_skipbias == 0)
...
class CNNBlock(nn.Module):
    ...
    def __init__(
        ...
        self.matcher = (
            nn.Conv2d(self.channels_in, self.channels_out, 1, padding=0, bias=False)
        ...
        self.conv1 = nn.Conv2d(
            self.channels_in, self.channels_internal, 1, padding=0, bias=False
        )
        ...
        self.conv3 = nn.Conv2d(
            self.channels_internal, self.channels_out, 1, padding=0, bias=True
        )
...
class GlobalBlock(nn.Module):
    ...
    def __init__(
        ...
        self.activation = activation
        self.activation_out = (
            activation_out if activation_out is not None else activation
        )
        self.reduction = reduction
        self.residual = residual
        ...
        self.latent_1x1 = (
            self.out_channels * self.internal_channels + self.internal_channels
        )
        self.latent_3x3 = (
            self.internal_channels * self.internal_channels * kernel_size * kernel_size
            + self.internal_channels
        )
        self.latent_1x1_out = (
            self.internal_channels * self.out_channels + self.out_channels
        )
        self.context_size = (self.patch_dim * self.num_patches) + (
            self.shared_context * 3
        )

        self.projector = (
            nn.Linear(self.out_channels * (self.patch_size**2), self.patch_dim)
            if projector is None
            else projector
        )

        self.conv_1x1 = nn.Linear(
            self.context_size, self.latent_1x1 + self.shared_context
        )
        self.conv_3x3 = nn.Linear(
            self.context_size, self.latent_3x3 + self.shared_context
        )
        self.conv_1x1_out = nn.Linear(
            self.context_size, self.latent_1x1_out + self.shared_context
        )
        ...
        self.pos_embed = self.posemb_sincos_2d(
            self.num_patches_height, self.num_patches_width, self.patch_dim
        )
        ...
        self.cnn_norm_1x1 = nn.LayerNorm(
            [self.internal_channels, self.chw[1], self.chw[2]]
        )
        self.cnn_norm_3x3 = nn.LayerNorm(
            [self.internal_channels, self.chw[1], self.chw[2]]
        )
    ...
    def convolve(self, x, context, in_channels, out_channels, size=3):
        ...
        _kernel, _bias = torch.split(
            context, [context.size(1) - out_channels, out_channels], dim=1
        )
        ...
        x = F.conv2d(
            x, kernel, groups=batch_size, bias=bias
    ...
    def forward(self, x, ctx_p=None):
        identity = (
            x if self.in_channels == self.out_channels else self.matcher(x)
        )  # shape: (B, C, H, W)
        x = self.norm_input(identity)

        ctx_p = (
            torch.zeros((x.shape[0], self.shared_context * 3), device=x.device)
            if ctx_p is None
            else ctx_p
        )  # shape: (B, shared_context)
        prev_1x1, prev_3x3, prev_1x1_out = ctx_p.split(
            [self.shared_context, self.shared_context, self.shared_context], dim=1
        )

        embedded_patches = self.forward_stem(x)  # shape: (B, num_patches * patch_dim)

        # 1x1 Convolution with global context
        inputs = self.context_norm_1x1(
            torch.cat([embedded_patches, prev_1x1, prev_3x3, prev_1x1_out], dim=1)
        )
        combined_context = self.conv_1x1(inputs)
        ctx, ctx_1x1 = combined_context.split(
            [self.latent_1x1, self.shared_context], dim=1
        )
        ...
        inputs = self.context_norm_3x3(
            torch.cat([embedded_patches, ctx_1x1, prev_3x3, prev_1x1_out], dim=1)
        )
        combined_context = self.conv_3x3(inputs)
        ctx, ctx_3x3 = combined_context.split(
            [self.latent_3x3, self.shared_context], dim=1
        )
        x = self.convolve(
            x,
            ctx,
        ...
        # 1x1 Convolution with global context
        inputs = self.context_norm_1x1_out(
            torch.cat([embedded_patches, ctx_1x1, ctx_3x3, prev_1x1_out], dim=1)
        )
        combined_context = self.conv_1x1_out(inputs)
        ctx, ctx_1x1_out = combined_context.split(
            [self.latent_1x1_out, self.shared_context], dim=1
        )
        x = self.convolve(x, ctx, self.internal_channels, self.out_channels, size=1)

        # Merge contexts
        ctx_o = self.context_shared_norm(
            torch.cat([ctx_1x1, ctx_3x3, ctx_1x1_out], dim=1)
        )
